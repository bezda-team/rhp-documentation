...
import { PlotContext, Scale, BoxAndWhiskerPlot, convertBWData, bPChangeBWOrder, orderBWsByPosition, BW_PLOT_DEFAULT_TEMPLATE } from '@bezda/rhp-core';
...
//   useSelector(//   const currValue = value);
//   return (
  ...
  const trackedBoxWhiskersConfig = useObservable(() => {
      ...
      untrackedData.forEach((value, i) => {
          newBarsDataTemp.push({
                                ...
                                elements: opaqueObject(BW_PLOT_DEFAULT_TEMPLATE),  // Avoid strange unexplainable circular reference errors for each element of this array on first render
...
export const InteractiveBoxAndWhiskerPlotExample = () => {
  ...
  const currentIndex = useSelector(index);
  const currentcssObservable = useSelector(cssObservable);
