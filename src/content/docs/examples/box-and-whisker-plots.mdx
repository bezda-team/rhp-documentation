...
- Both the `<Scale>` and `<BoxAndWhiskerPlot>` components are wrapped in a `<PlotContext.Provider>` component. This is necessary for the components to share the same state observables (provided by the context).
