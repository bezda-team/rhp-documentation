...
<Tabs>
  ...
  <div class="component-guide-s-plot grid justify-center ">
    <TutorialSimpleBarPlotA />
  </div>
  </TabItem>
</Tabs>

In the above, we create our `plotData`, `vars`, and `dataMax` observables as local (to the App component) observables using the `useObservable` hook from `@legendapp/state/react`. 
We give them some initial data. Note that we are not providing a template to the `<SegmentPlot>` component and so it will use a built-in default template.
...
the `<SegmentPlot>` component in some container element to help with placement and scaling
:::
